...
const { buildResetPasswordUrl } = require('../lib/requestUrl');

const JWT_SECRET = process.env.JWT_SECRET;
const SALT_ROUNDS = 12;
const TOKEN_EXPIRY = '24h';
const OWNER_SETUP_CODE = process.env.SIDURI_OWNER_SETUP_CODE || process.env.INITIAL_OWNER_SETUP_CODE || '';
...
router.post('/register', async (req, res) => {
  try {
    ...
    const createUser = db.transaction(() => {
      ...
      if (isFirstUser) {
        if (!OWNER_SETUP_CODE) {
          return { status: 503, error: 'Owner setup code is not configured' };
        }
        if (!safeSecretEquals(inviteCode, OWNER_SETUP_CODE)) {
          return { status: 400, error: 'Invalid owner setup code' };
        }
      } else {
        if (!inviteCode) {
          return { status: 400, error: 'Invitation code required' };
        }
        ...
        if (!invitation) {
          return { status: 400, error: 'Invalid or expired invitation code' };
        }

        if (invitation.email && normalizeEmail(invitation.email) !== email) {
          return { status: 400, error: 'Invitation is for a different email address' };
        }

        const consumedInvitation = db.prepare(`
          UPDATE invitations
        ...
        if (consumedInvitation.changes !== 1) {
          return { status: 400, error: 'Invalid or expired invitation code' };
        }
      }
      ...
      db.prepare(`
        INSERT INTO users (id, email, password_hash, name, role, session_version)
        VALUES (?, ?, ?, ?, ?, 0)
      `).run(userId, email, passwordHash, name || null, role);

      return {
        user: { id: userId, email, name: name || null, role, session_version: 0 }
      };
    });
...
router.post('/logout', jwtAuth, (req, res) => {
  try {
    ...
    const expiresAt = toSqliteDateTime(new Date(Date.now() + 24 * 60 * 60 * 1000));
    db.prepare(`
...
router.get('/check-first-user', (req, res) => {
  try {
    ...
    res.json({
      ...
      ownerSetupConfigured: userCount.count > 0 || Boolean(OWNER_SETUP_CODE)
    });
  } catch (error) {
...
router.post('/invitations', jwtAuth, (req, res) => {
  try {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).run(code, email, user.id, expiresAt);
...
router.get('/invitations', jwtAuth, (req, res) => {
  try {
    ...
    const invitations = db.prepare(`
      SELECT code, email, created_by, used_at, expires_at, created_at
      FROM invitations
      ORDER BY created_at DESC
    `).all();
