...
function jwtAuth(req, res, next) {
  ...
  try {
    ...
    const user = db.prepare('SELECT id, email, name, role, session_version FROM users WHERE id = ?').get(decoded.userId);
    if (!user) {
      return res.status(401).json({ error: 'User not found' });
    }

    if ((decoded.sv || 0) !== (user.session_version || 0)) {
      return res.status(401).json({ error: 'Token has been revoked' });
    }
...
function optionalAuth(req, res, next) {
  ...
  try {
    ...
    if (!revoked) {
      const user = db.prepare('SELECT id, email, name, role, session_version FROM users WHERE id = ?').get(decoded.userId);
      if (user && (decoded.sv || 0) === (user.session_version || 0)) {
        req.user = user;
      } else {
        req.user = null;
      }
