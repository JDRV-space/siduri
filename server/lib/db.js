...
try {
  db.exec(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_pending_uploads_object_path ON pending_uploads(object_path);
  `);
  ...
  const invitationColumns = db.pragma('table_info(invitations)').map(c => c.name);
  if (!invitationColumns.includes('created_at')) {
    db.exec('ALTER TABLE invitations ADD COLUMN created_at TEXT');
    db.exec("UPDATE invitations SET created_at = COALESCE(created_at, datetime('now'))");
  }

  const ownerCount = db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'owner'").get();
  if (ownerCount.count > 1) {
    throw new Error('Multiple owner users exist. Resolve duplicate owners before starting Siduri.');
  }
