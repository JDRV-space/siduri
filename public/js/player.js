...
async function init() {
  try {
    // Fetch video data
    const videoApiUrl = new URL(`api/videos/${videoId}`, document.baseURI);
    if (viewerToken) {
      videoApiUrl.searchParams.set('v', viewerToken);
    }
...
async function loadSubtitles() {
  if (subtitlesLoaded || !videoData?.subtitleUrl) return;

  try {
    const res = await fetch(videoData.subtitleUrl);
    if (!res.ok) {
      return;
    }
